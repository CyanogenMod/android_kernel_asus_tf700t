...
static int tegra3_cpu_clk_set_rate(struct clk *c, unsigned long rate)
{
	...
	if (c->dvfs)
		if (c->dvfs->dvfs_rail){
				if((!c->dvfs->dvfs_rail->reg) && (!strncmp(c->dvfs->dvfs_rail->reg_id,"vdd_cpu",7))){
					c->dvfs->dvfs_rail->reg=regulator_get(NULL, "vdd_cpu");
					printk("[Warning]tegra3_cpu_clk_set_rate set vdd_cpu as power source for %s reg=%p\n",c->name,c->dvfs->dvfs_rail->reg);
				}
		}
	if (c->dvfs) {
		if (!c->dvfs->dvfs_rail)
...
static struct tegra_cpufreq_table_data cpufreq_tables[] = {
	...
	{ freq_table_1p5GHz, 2, 10, 3},//base on 1P3
	{ freq_table_1p6GHz, 2, 11, 3},//base on 1P4
	{ freq_table_1p7GHz, 2, 12, 3},
	{ freq_table_1p8GHz, 2, 12, 3},//base on 1P7
	{ freq_table_1p9GHz, 2, 12, 3},//base on 1P7
};

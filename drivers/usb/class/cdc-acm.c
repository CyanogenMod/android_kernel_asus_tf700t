...
#else
		/*
		 * Asus: This is only needed when usb_anchor_urb()
		 * is not used.
		 */
		if (!acm->delayed_wb)
			acm->delayed_wb = wb;
		     ...
		     const struct usb_device_id *id)
{
	...
	/* Don't bind network interface on IMC XMM6260 */
...
skip_countries:
	...
	/*Begin: Added for selective suspend */
	if (usb_dev->descriptor.idVendor == 0x1519 &&
	...
	/*End: Added for selective suspend */
